...
static Handle<Value> Countly_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Countly.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::CountlyBindings::lookupGeneratedInit(
...
static void Countly_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Countly_getBinding)->GetFunction());
}

static void Countly_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::CountlyBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry CountlyBinding = {
	"ly.count",
	Countly_init,
	Countly_dispose
};
...
Java_ly_count_CountlyBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ly.count", &CountlyBinding);
	titanium::KrollBindings::addExternalLookup(&(::CountlyBindings::lookupGeneratedInit));
}
